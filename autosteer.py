...
def play_video(video_path, total_files, idx, autosteer, screen_w, screen_h, writer=None):
...
def main():
	...
	probe.release()
	out_path = os.path.join(cwd, "autosteer.m4v")
	...
	try:
		for i, vp in enumerate(videos, start=1):
			if not play_video(vp, len(videos), i, autosteer, screen_w, screen_h, writer=writer):
				break
	finally:
		writer.release()
